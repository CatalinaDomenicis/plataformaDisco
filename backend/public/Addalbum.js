...
async function loadAlbums() {
    try {
        const response = await axios.get('http://localhost:5000/band/band');
...
document.getElementById('album-form').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await axios.post('http://localhost:5000/band/band', albumData);
...
document.getElementById('guardarcambios').addEventListener('click', async () => {
    ...
    try {
        const response = await axios.put(`http://localhost:5000/band/band/${editingAlbumId}`, albumData);
...
async function deleteAlbum(id) {
    if (confirm('¿Estás seguro de que quieres eliminar este álbum?')) {
        try {
            await axios.delete(`http://localhost:5000/band/band/${id}`);
...
document.getElementById('addSongButton').addEventListener('click', async() => {
    ...
    axios.post(`http://localhost:5000/band/${currentAlbumId}/songs`, { titulo: songName, duracion: songDuration })
      .then(() => {
...
function deleteSong(albumId, songId) {
    axios.delete(`http://localhost:5000/band/${albumId}/songs/${songId}`)
      .then(() => {
